...
import { Download, Filter, FileBarChart2, Coins, TrendingUp, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button className="flex items-center gap-2 px-5 py-2.5 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 transition-all bg-white shadow-sm">
            <Download size={16} /> Exportar PDF
          </button>
          <button className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-black transition-all shadow-xl shadow-slate-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Riesgo Obsolescencia</p>
          ...
          <p className="text-[10px] text-slate-400 font-bold leading-tight mt-4 italic">
            * Activos con más de 3 años de antigüedad técnica.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-full uppercase tracking-widest">Finanzas</span>
          </div>
          <div className="h-80">
          ...
          {topAssets.map((item, i) => (
            <div key={item.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all group">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <p className="font-bold text-slate-100">{item.equipo}</p>
                    <p className="text-xs text-slate-400 uppercase font-bold tracking-tighter">{item.codigo} • {item.empresa}</p>
                 </div>
              </div>
              <div className="text-right">
                 <p className="text-lg font-black text-blue-400">{item.coste}</p>
                 <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">{item.asignado}</p>
              </div>
            </div>
          ))}
        </div>
