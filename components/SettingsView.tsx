...
const SettingsView: React.FC<SettingsViewProps> = ({ catalog, setCatalog, sheetUrl, setSheetUrl, onCatalogUpdate }) => {
  ...
  const handleRemoveItem = (itemToRemove: string) => {
    if (activeCategory === 'cloud') return;
    if (window.confirm(`¿Eliminar "${itemToRemove}" del catálogo maestro?`)) {
  ...
  const handleSaveUrl = () => {
    setSheetUrl(tempUrl);
    localStorage.setItem('zubi_sheet_url', tempUrl);
    alert('URL de sincronización actualizada correctamente.');
  };

  const handleClearCache = () => {
    if (window.confirm("¿Seguro que deseas borrar el cache local? Se recargarán todos los registros desde Google Sheets.")) {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row min-h-[750px]">
        {/* Sidebar de Ajustes */}
        <div className="w-full md:w-80 bg-slate-50 border-r border-slate-200 p-8 flex flex-col overflow-y-auto max-h-[750px] custom-scrollbar">
          ...
          <div className="mt-8 pt-8 border-t border-slate-200">
             <button onClick={handleClearCache} className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-rose-500 hover:bg-rose-50 transition-all">
            ...
            <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-sm font-medium mt-3">Gestiona el enlace directo con el libro de Google.</p>
                  </div>
               </div>
               ...
               <div className="bg-slate-900 p-10 rounded-[2.5rem] text-white space-y-8 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <h4 className="font-black text-xs uppercase tracking-[0.2em] text-blue-400 mb-2">Google Apps Script Web App</h4>
                    <p className="text-white/60 text-xs font-bold leading-relaxed mb-6">Pega aquí la URL de ejecución generada al "Implementar como Aplicación Web".</p>
                    <div className="space-y-4">
                       ...
                       >
                          <ShieldCheck size={20}/> Validar y Guardar Endpoint
                       </button>
                    </div>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="p-8 bg-white border-2 border-slate-100 rounded-[2rem] shadow-sm">
                     <h5 className="font-black text-xs text-slate-900 uppercase tracking-widest mb-4 flex items-center gap-3">
                        <ExternalLink size={18} className="text-blue-500"/> Recomendaciones
                     </h5>
                     <ol className="text-xs text-slate-500 space-y-3 font-bold leading-relaxed">
                        <li className="flex gap-3"><span className="text-blue-600">01.</span> Use pestañas "inventario" y "catalogo".</li>
                        <li className="flex gap-3"><span className="text-blue-600">02.</span> No cambie el orden de las 38 columnas.</li>
                        <li className="flex gap-3"><span className="text-blue-600">03.</span> Configure acceso "Cualquier persona" en GAS.</li>
                     </ol>
                  </div>
                  <div className="p-8 bg-blue-50 border-2 border-blue-100 rounded-[2rem] flex flex-col justify-center">
                     <div className="flex items-center gap-3 mb-3">
                        <Info className="text-blue-600" size={20} />
                        <h5 className="font-black text-xs text-blue-900 uppercase tracking-widest">Capacidad Detectada</h5>
                     </div>
                     <p className="text-[11px] text-blue-900/60 font-bold leading-relaxed italic">
                        Sistema optimizado para manejar +1000 registros simultáneos con latencia reducida.
                     </p>
                  </div>
               </div>
            </div>
            ...
            <div className="animate-in slide-in-from-right-4 duration-500 flex flex-col h-full">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="relative">
                   ...
                   <input 
                      ...
                      className="pl-10 pr-6 py-2 bg-slate-100 border-none rounded-full text-xs font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                   />
                </div>
              </div>
              ...
              <form onSubmit={handleAddItem} className="mb-8 flex gap-4">
                <input
                  ...
                  placeholder={`Nueva opción en ${categories.find(c => c.key === activeCategory)?.label}...`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-10">
                  ...
                  {filteredItems.length === 0 && (
                     <div className="col-span-full py-24 text-center flex flex-col items-center gap-4">
                        ...
                        <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.3em]">No hay elementos registrados en esta categoría</p>
                     </div>
                  )}
                </div>
              </div>
